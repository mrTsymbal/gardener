...
function ale_add_scripts($hook) {
    ...
    if (in_array($hook, array('appearance_page_aletheme_theme_demos'))) {
        ...
        wp_localize_script( 'aletheme-demos', 'olins_strings', array(
            'installingPlugin' => esc_html__( 'Installing plugin', 'olins' ),
            'activatingPlugin' => esc_html__( 'Activating plugin', 'olins' ),
	        'tryAgain' => esc_html__('Try again', 'olins'),
            ...
            'plugin_failed_activation' => esc_html__( 'Failed to activate.', 'olins' ),
            'plugin_active' => esc_html__( 'Active', 'olins' ),
            'plugin_failed_activation_retry' => esc_html__( 'Failed.', 'olins' ),
            'plugin_failed_activation_memory' => esc_html__( 'The plugin failed to activate. Please try to increase the memory_limit on your server.', 'olins' ),
            'content_importing_error' => esc_html__( 'There was a problem during the importing process resulting in the following error from your server:', 'olins' ),
            'required_plugins' => esc_html__('Install required plugins first.', 'olins'),
        ) );
    }
...
function ale_add_admin_menu() {
    add_theme_page( esc_html__('Theme Options', 'olins'), esc_html__('Theme Options', 'olins'), 'edit_theme_options', 'aletheme','optionsframework_page');
...
function ale_ajax_post_install_plugin() {
    ...
    if ( $plugin_installed === true ) {
        ...
        echo sprintf( __( 'The plugin failed to install. Please check the permissions for the "plugins" directory or <a href="%s" target="_blank">download</a> the plugin and install it manually.', 'olins' ), $plugin_source );
    }
    die();
}
