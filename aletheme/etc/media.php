...
function ale_attachment_fields_to_edit($form_fields, $post) {
	...
	if (function_exists('ale_media_custom_fields')) {
		foreach (ale_media_custom_fields() as $field) {
			$data = array();
...
function ale_gallery_metabox_add() {
	...
	if (function_exists('ale_get_post_types_with_gallery')) {
		$post_types = ale_get_post_types_with_gallery();
	} else {
	...
	foreach ($post_types as $post_type) {
		...
		if( apply_filters( 'ale_gallery_metabox_limit', true, $post_id ) ) {
			add_meta_box( 'ale_gallery_metabox', esc_html__( 'Gallery Images', 'olins' ), 'ale_gallery_metabox', $post_type, $context, $priority );
		}
	}
}
...
function ale_gallery_metabox_html($post_id) {
    ...
    $intro = '<p><a href="media-upload.php?post_id=' . $post_id .'&amp;type=image&amp;TB_iframe=1&amp;width=640&amp;height=715" id="add_image" class="thickbox" title="' . esc_html__( 'Add Image', 'olins' ) . '">' . esc_html__( 'Upload Images', 'olins' ) . '</a> | <a href="media-upload.php?post_id=' . $post_id .'&amp;type=image&amp;tab=gallery&amp;TB_iframe=1&amp;width=640&amp;height=715" id="manage_gallery" class="thickbox" title="' . esc_html__( 'Manage Gallery', 'olins' ) . '">' . esc_html__( 'Manage Gallery', 'olins' ) . '</a></p>
    ...
    $loop = get_posts($args);
    if (empty($loop)) {
        return '<p>Gallery is empty. You must upload new images. You are not able to use old images uploaded from Media Library. <b style="color:#990000;">After uploading close the modal window, don\'t insert images into post.</b></p><a href="media-upload.php?post_id=' . $post_id .'&amp;type=image&amp;TB_iframe=1&amp;width=640&amp;height=715" id="add_image" class="thickbox" title="' . esc_html__( 'Add Image', 'olins' ) . '">' . esc_html__( 'Upload Images', 'olins' ) . '</a>';
    }
...
function ale_gallery_metabox_do_ajax_update( $form_fields, $post ) {
	$js = '<script type="text/javascript">
	    ...
	    jQuery.ajax({
		    ...
		    type: "POST",
		    data: "action=refresh_metabox&post_id=<?php echo esc_attr($post->post_parent); ?>&current_post_id=" + jQuery(\'#post_ID\', top.document).val(),
