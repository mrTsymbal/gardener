...
function ale_get_recent_from_instagram($login = '',$access_token = '',$count = ''){
    ...
    if(empty($login) or empty($token) or empty($count) or empty($thumb)) {
        ...
        $error = esc_html__('Fill all the Instagram Feed fields in Theme Options Panel','olins');
    ...
    $html .= '<div class="ale_insta_subscribe"><span class="button_border">'.esc_html__('Follow us on Instagram','olins').' <a href="'.esc_url($subsribe_link).'" target="_blank">'.esc_attr($login).'</a></span></div>';
